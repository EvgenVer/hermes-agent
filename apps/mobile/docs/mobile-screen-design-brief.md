...
**Audience:** Google Stitch, Claude Design, or another UI-generation tool.
...
- Authentication: Hermes authentication plus a short-lived WebSocket ticket;
  credentials and tokens are never ordinary UI state.
...
7. **Forms protect work.** Dirty forms warn before navigation, disconnect,
   lock, or loss of connection. Never silently discard edits.
...
- a selected-agent context when relevant;
...
| Dirty form | Unsaved-change warning before leaving | Stay, discard, or save |
...
- States: waiting for provider, browser/native handoff if required, success,
  invalid credentials, provider unavailable, cancelled.
...
- Full-screen multiline editor with readable line height and character/size
  guidance only if server contract provides a limit.
...
- Include explanatory copy that capability changes apply after server
  confirmation.
...
- Show a warning before changes that affect agent capabilities if the server
  contract requires it.

#### E07 — MCP servers (P1)

- Redacted server list: display name, type/category, enabled state, connection
  or last-test status.
- Actions: `Add server`, `Test connection`, `Enable`, `Remove`.
...
- Fields: name, schedule, enabled state, delivery/behavior options only when
  supported by the server contract.
- Explicit preview of `Next run` when possible.
...
- Trigger before back, tab switch, disconnect, lock, or connection loss.
...
- Show local URI only as a temporary chip; after upload show safe filename,
  type, size, and upload state.
- Enforce and display limits: images up to 25 MiB, PDFs up to 50 MiB/25 pages;
  ...
  it.
- Variants: unsupported type, too large, upload failure, pending attachment,
  send disabled while invalid.

#### C05 — History and resume

- Full-screen or bottom-sheet history scoped to the selected profile/session.
- Show ordered message/session rows, pagination affordance, and loading state.
- Highlight current canonical `Bot Chat` without using recency to choose it.
...
- Show available choices or a text response field, required/optional status,
  and `Submit answer`.
- Correlate to the visible session and preserve exactly-once submission.

#### C08 — Sudo request

- Explain why elevated execution requires explicit approval without exposing
  hidden command details beyond the safe server-provided summary.
- Actions: explicit allow/deny or contract-provided choices.
- Use the same expiry/stale/submitted variants as approval.
...
- Clear copy: `This value is sent to Hermes for this request and is not shown
  in the conversation.`
- Actions: `Submit securely`, `Cancel` if the server contract allows.
- After submit, clear the field and disable duplicate submission.
...
- Provide `Refresh status` and link to `Logs`.

#### O03 — Logs

- Authenticated, paginated log viewer with level/component/search filters.
- Monospace rows are acceptable, but preserve readability on a phone.
...
- Explain that the connection will briefly close and the app will wait for
  health recovery; a fully stopped Hermes process cannot restart itself.
- Actions: `Cancel`, destructive/service action `Restart gateway`.
- Progress sequence: requested → connection closed → waiting for Hermes →
  reconnected → status refreshed; timeout/error state must remain honest.
...
- Keep destructive `Disconnect and forget` visually separated.
...
- Explain that lock-screen notifications are generic by default.
- Handle permission denied, revoked registration, and offline states.
...
7. Operations → Server status → Logs → Restart gateway → disconnect/recovery →
   refreshed status.
...
least-privilege log/status projection, mobile reconnect/replay, secure local
cache, and generic-file attachment limits.
...
## 11. Deliverables expected from Stitch / Claude Design
