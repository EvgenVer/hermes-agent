...
- [ ] HM-001 Fetch pruned origin and upstream references — files: Git references only; no working-tree files · verify: git fetch --prune origin and git fetch --prune upstream both succeed; origin/main, upstream/main, and feat/mobile-app resolve to commits · dep: none · parallel: no
- [ ] HM-002 Decide whether local main can fast-forward to upstream/main — files: apps/mobile/docs/upstream-baseline.md · verify: evidence records the three SHAs, ahead/behind counts, merge-base, dirty-state check, and an explicit fast-forward-safe or blocked decision · dep: HM-001 · parallel: no
- [ ] HM-003 Fast-forward local main to upstream/main when HM-002 records fast-forward-safe — files: local main Git reference only · verify: git rev-parse main equals git rev-parse upstream/main and git status --short is empty · dep: HM-002 · parallel: no
- [ ] HM-004 Merge upstream/main into feat/mobile-app without rebasing or rewriting shared history — files: Git merge commit and files changed by the bounded upstream diff · verify: git merge-base --is-ancestor upstream/main HEAD succeeds; git diff --check succeeds; checks for upstream-touched packages pass · dep: HM-003 · parallel: no
